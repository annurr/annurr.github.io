...
async function fetchWithProgress(url, onProgress, onText) {
    // ----------------------------------------------------
    // Bypass GitHub/Azure Blob CORS blocks on large binaries using our streaming Edge Function proxy
    if (url.includes('github.com') || url.includes('githubusercontent.com') && !url.includes('objects.githubusercontent.com')) {
        url = `https://svhwsfzvaotanzbcwcez.supabase.co/functions/v1/github-release-proxy?url=${encodeURIComponent(url)}`;
    }
    // ----------------------------------------------------
